...
GeoNetwork.FacetsPanel = Ext.extend(Ext.Panel, {
    ...
    displayFacetValue: function (node, visible) {
        ...
        if (this.currentFilterStore.getCount() === 0 ||
            ...
            var recId = this.facetsStore.getCount() + 1,
                 ...
                 isDatatypeKeyword = (data.facet === 'datatypekeyword');
            if(isSiteOrRegionKeyword){
            	if(data.label != null){
     ...
     */
    addFacet: function (recordId, sitekeywordData) {
        var r = this.facetsStore.getById(recordId),
            form = this.searchForm;
        var id = 'facet_' + (sitekeywordData ? 0 : this.counter++); 
        var data = sitekeywordData || {
            id: id, 
            facet: r.get('facet'), 
            value: r.get('node'), 
        ...
        var filter = new this.currentFilterStore.recordType(data, /*this.currentFilterStore.getCount() + 1*/sitekeywordData ? 0 : this.counter - 1);
 ...
 */
            ...
            this.breadcrumb.add(new Ext.Button({
                ...
                listeners: {
                    removed: function(comp, ownerCt) {
                        var textComp = Ext.getCmp(comp.id + '_text');
                        this.breadcrumb && this.breadcrumb.remove(textComp);
                    },
    				scope:panel
                },
                scope: panel
    ...
    removeFacet: function (filterKey, silent) {
        // Search in current filter
    	var records = this.currentFilterStore.query('id', filterKey);
    	if (records.length>0) {
            var filter = records.get(0);

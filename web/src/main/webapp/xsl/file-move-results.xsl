...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<html>
		...
		<body>
		<center>
			<h2><xsl:value-of select="/root/gui/strings/fileMoveSuccessful"/></h2>
			<p/>
			<table>
				<tr>
					<th><xsl:value-of select="concat(/root/gui/strings/file,': ')"/></th>
					<td><span id="filename_moved" title="{/root/response/fname}"><xsl:value-of select="/root/response/fname"/></span></td>
				</tr>
				<tr>
					<th><xsl:value-of select="concat(/root/gui/strings/fileType,': ')"/></th>
					<td align="right"><span id="filetype_moved" title="{/root/response/ftype}"><xsl:value-of select="/root/response/ftype"/></span></td>
				</tr>
				<!-- <tr>
